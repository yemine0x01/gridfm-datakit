...
def _make_config(tmp_dir: str) -> NestedNamespace:
    """Build a minimal valid config pointing at CSVs in tmp_dir."""
    models_df = pd.DataFrame(
        {
            "static_id": ["GEN1", "GEN2"],
            "dynamic_model_id": ["GeneratorSynchronousThreeWindings"] * 2,
            "parameter_set_id": ["p0", "p1"],
        },
    )
    events_df = pd.DataFrame(
        {
            ...
            "parameter_set_id": ["ev0"],
        },
    )
    variables_df = pd.DataFrame(
        {
            "dynamic_model_id": ["GEN1"],
            "variable": ["generator_omegaPu"],
        },
    )

    models_path = os.path.join(tmp_dir, "models.csv")
    events_path = os.path.join(tmp_dir, "events.csv")
    variables_path = os.path.join(tmp_dir, "variables.csv")

    _write_csv(models_path, models_df)
    _write_csv(events_path, events_df)
    _write_csv(variables_path, variables_df)
...
class TestLoadCsvFormat:
    def test_returns_dynamic_inputs(self, tmp_path):
        config = _make_config(str(tmp_path))
        result = load_raw_inputs(config)
        assert isinstance(result, DynamicInputs)

    def test_dynamic_models_shape(self, tmp_path):
        config = _make_config(str(tmp_path))
        result = load_raw_inputs(config)
        assert len(result.dynamic_models) == 2
        assert "static_id" in result.dynamic_models.columns

    def test_events_shape(self, tmp_path):
        config = _make_config(str(tmp_path))
        result = load_raw_inputs(config)
        assert len(result.events) == 1

    def test_variables_shape(self, tmp_path):
        config = _make_config(str(tmp_path))
        result = load_raw_inputs(config)
        assert len(result.variables) == 1

    def test_all_required_model_columns_present(self, tmp_path):
        config = _make_config(str(tmp_path))
        result = load_raw_inputs(config)
        for col in DYNAMIC_MODELS_REQUIRED_COLS:
            assert col in result.dynamic_models.columns

    def test_all_required_event_columns_present(self, tmp_path):
        config = _make_config(str(tmp_path))
        result = load_raw_inputs(config)
        for col in EVENTS_REQUIRED_COLS:
            assert col in result.events.columns

    def test_all_required_variable_columns_present(self, tmp_path):
        config = _make_config(str(tmp_path))
        result = load_raw_inputs(config)
...
class TestLoadRawInputsErrors:
    def test_missing_file_raises(self, tmp_path):
        config = NestedNamespace(
            dynamic=NestedNamespace(
                dynamic_solver="dynawo",
                dynamic_models_file=str(tmp_path / "nonexistent.csv"),
    ...
    def test_missing_column_raises_value_error(self, tmp_path):
        ...
        bad_models.to_csv(str(tmp_path / "models.csv"), index=False)
        ...
        config = NestedNamespace(
            dynamic=NestedNamespace(
                dynamic_solver="dynawo",
                dynamic_models_file=str(tmp_path / "models.csv"),
                events_file=str(tmp_path / "events.csv"),
                variables_file=str(tmp_path / "variables.csv"),
            ),
        )
